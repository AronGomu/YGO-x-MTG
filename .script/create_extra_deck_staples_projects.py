...
CARDS = {
    "Fusion": [
        ...
        ("Garura, Wings of Resonant Life", "<i>Fusion — 2 créatures avec le même type et attribut mais des noms différents</i>", ["(1 - Passif) Vol.", "(2 - Déclenchable) <b>On Send Grave</b> — Piochez une carte."]),
        ("Elder Entity N'tss", "<i>Fusion — 1 Synchro Creature + 1 Xyz Creature</i>", ["(1 - Déclenchable) Si Elder Entity N'tss arrive, vous pouvez mettre une créature depuis votre extra deck dans votre Grave.", "(2 - Déclenchable) <b>On Send Grave</b> — Détruisez le permanent ciblé."]),
    ...
    "Synchro": [
        ...
        ("Borreload Savage Dragon", "<i>Synchro — 1 Tuner + 1+ non-Tuner</i>", ["(1 - Déclenchable) Si Borreload arrive, exilez une Link Creature de votre Grave. Mettez X marqueurs Borrel sur Borreload, où X est son Link.", "(2 - Activable Flash) Retirez un marqueur Borrel : contrecarrez un sort ou une capacité ciblé."]),
        ("Chaos Angel", "<i>1 Tuner + 1+ non-Tuner blanc ou noir</i>", ["(1 - Déclenchable) **On Enter** — Ciblez 1 permanent non-terrain ; exilez-le.", "(2 - Passif) Si 1 matériel noir a été utilisé, Chaos Angel gagne **Indestructible**. Si 1 matériel blanc a été utilisé, il gagne **Protection contre tout**."]),
        ("Swordsoul Supreme Sovereign - Chengying", "<i>Synchro — 1 Tuner + 1+ non-Tuner</i>", ["(1 - Passif) Chengying gagne +1/+1 pour chaque carte exilée et les créatures adverses gagnent -1/-1 pour chaque carte exilée par vous ce tour-ci.", "(2 - Déclenchable Soft) Si une carte est exilée, exilez une carte ciblée sur le champ de bataille et une carte ciblée d'un Grave."]),
        ("PSY-Framelord Omega", "<i>Synchro — 1 Tuner + 1+ non-Tuner</i>", ["(1 - Activable Flash Soft) Exilez Omega et une carte aléatoire de la main d'un adversaire jusqu'au début de votre prochain entretien.", "(2 - Activable Sorcery Soft) Remettez une carte exilée dans le Grave de son propriétaire."]),
        ...
        ("Accel Synchro Stardust Dragon", "<i>Synchro — 1 Tuner + 1+ non-Tuner</i>", ["(1 - Déclenchable) Si Accel Synchro arrive, renvoyez une créature Tuner ciblée depuis votre Grave sur le champ de bataille.", "(2 - Activable Flash Soft) Exilez Accel Synchro : invoquez une Synchro Creature depuis votre extra deck en utilisant vos créatures comme matériaux."]),
        ("Stardust Dragon", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Activable Flash Soft) Sacrifiez Stardust Dragon et ciblez 1 sort ou capacité qui détruirait 1 ou plusieurs cartes : **Negate & Destroy**. Au début de la prochaine étape de fin : **Reanimate** cette carte."]),
        ("Black Rose Dragon", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Déclenchable) **On Enter** — Vous pouvez détruire toutes les cartes non-terrain.", "(2 - Activable Ritual Soft) **Exile 1 Plant from Grave** : 1 créature ciblée ne peut pas bloquer ce tour-ci."]),
        ("Black Rose Moonlight Dragon", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Déclenchable) **On Enter or MV2+ Opponent Creature Enter** — Ciblez 1 créature d’un adversaire ; **Bounce**."]),
        ("Trishula, Dragon of the Ice Barrier", "<i>1 Tuner + 2+ non-Tuner</i>", ["**Vol**", "(1 - Déclenchable) **On Enter** — Exilez jusqu’à 1 carte depuis la main, le terrain et le **Grave** d’un adversaire."]),
        ("Brionac, Dragon of the Ice Barrier", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Activable Sorcery Soft) Défaussez X cartes ; renvoyez X permanents ciblés dans la main de leurs propriétaires."]),
        ("Coral Dragon", "<i>1 Tuner + 1+ non-Tuner</i>", ["(1 - Activable Sorcery Soft) Défaussez 1 carte et ciblez 1 permanent non-créature ; détruisez-le.", "(2 - Déclenchable Hard) **On Send Grave** — Piochez 1 carte."]),
        ("Formula Synchron", "<i>1 Tuner + 1 non-Tuner</i>", ["(1 - Déclenchable) **On Enter** — Piochez 1 carte.", "(2 - Activable Flash Soft) Effectuez une invocation Synchro d’1 créature."]),
        ("Herald of the Arc Light", "<i>1 Tuner + 1+ non-Tuner</i>", ["(1 - Passif) Si 1 carte devait être mise dans un **Grave** depuis une main ou un Deck, exilez-la à la place.", "(2 - Déclenchable) **On Send Grave** — Cherchez 1 Ritual Creature ou 1 non-créature Ritual Summon."]),
        ("T.G. Hyper Librarian", "<i>1 Tuner + 1+ non-Tuner</i>", ["(1 - Déclenchable Soft) **On Enter Synchro** — Piochez 1 carte."]),
        ("Ancient Fairy Dragon", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Activable Sorcery Soft) **Hand Summon** 1 créature MV 1 ou moins.", "(2 - Activable Sorcery Soft) Détruisez autant d’enchantements que vous contrôlez. Gagnez 3 PV puis cherchez 1 enchantement de nom différent."]),
        ("Hot Red Dragon Archfiend Abyss", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Activable Flash Soft) Ciblez 1 créature d’un adversaire : **Negate** la cible ce tour-ci.", "(2 - Déclenchable) Si cette carte inflige des dommages de combat à un adversaire : ciblez 1 créature Tuner dans votre **Grave** ; **Reanimate**-la engagée."]),
        ("Red Supernova Dragon", "<i>3 Tuner + 1+ non-Tuner Synchro</i>", ["**Indestructible**, **Vol**", "(1 - Passif) Gagne +1/+0 pour chaque Tuner dans votre **Grave**.", "(2 - Activable Flash Soft) **On Opponent Activation or Attack** : vous pouvez exiler cette carte et si vous le faites, exilez tous les permanents non-terrain des terrains adverses. À votre prochaine étape de fin, **Summon** cette carte depuis l’exil."]),
        ("Scarlight Red Dragon Archfiend", "<i>1 Tuner + 1+ non-Tuner</i>", ["**Vol**", "(1 - Activable Sorcery Soft) Détruisez toutes les autres créatures avec une puissance inférieure à celle de cette carte, puis infligez 3 dommages à vos adversaires pour chaque créature détruite de cette façon."]),
        ("Goyo Guardian", "<i>1 Tuner + 1+ non-Tuner</i>", ["(1 - Déclenchable) Si Goyo Guardian détruit 1 créature d’un adversaire : **Reanimate** cette créature engagée."]),
    ],
    "Xyz": [
        ("Abyss Dweller", "<i>Xyz 2</i>", ["(1 - Activable Flash Soft) Détachez un matériau : les cartes dans les Graves adverses perdent toutes leurs capacités jusqu'à la fin du tour.", "(2 - Passif) Si Abyss Dweller a un matériau WATER, les créatures que vous contrôlez gagnent +1/+0."]),
        ...
        ("Divine Arsenal AA-ZEUS - Sky Thunder", "<i>1 Xyz Creature qui a combattu ce tour-ci</i>", ["(1 - Activable Flash) Détachez deux matériaux : mettez toutes les autres cartes non-terrain dans les Graves de leurs propriétaires.", "(2 - Déclenchable Soft) Si une autre carte que vous contrôlez est détruite, attachez une carte de votre main, champ de bataille ou extra deck à ZEUS comme matériau."]),
        ...
        ("Number 60: Dugares the Timeless", "<i>Xyz 2</i>", ["(1 - Activable Sorcery Soft) Détachez deux matériaux et choisissez un mode : piochez deux puis défaussez une carte ; renvoyez une créature depuis votre Grave ; ou doublez la force d'une créature jusqu'à la fin du tour. Sautez votre prochaine étape associée."]),
        ...
        ("Super Starslayer TY-PHON - Sky Crisis", "<i>1 Xyz Creature ou 2 créatures</i>", ["(1 - Passif) Tant que TY-PHON est sur le champ de bataille, les créatures avec une force de 3 ou plus ne peuvent pas activer leurs capacités.", "(2 - Activable Sorcery Soft) Détachez un matériau : renvoyez une créature ciblée dans la main de son propriétaire."]),
        ("The Phantom Knights of Break Sword", "<i>Xyz 2</i>", ["(1 - Activable Sorcery Soft) Détachez un matériau : détruisez une carte que vous contrôlez et une carte qu'un adversaire contrôle.", "(2 - Déclenchable) Si Break Sword est détruit, renvoyez jusqu'à deux créatures DARK depuis votre Grave engagées."]),
    ...
    "Link": [
        ...
        ("I:P Masquerena", "<i>Link 2</i>", ["(1 - Activable Flash Soft) Invoquez une Link Creature depuis votre extra deck en utilisant vos créatures comme matériaux.", "(2 - Passif) Une Link Creature invoquée avec I:P comme matériau a Indestructible des Effets."]),
        ...
        ("Accesscode Talker", "<i>Link 2+</i>", ["(1 - Déclenchable) Si Accesscode arrive, il gagne +X/+0 jusqu'à la fin du tour, où X est le Link d'une Link Creature ciblée dans votre Grave.", "(2 - Activable Sorcery) Exilez une Link Creature de votre Grave : détruisez une carte ciblée. N'activez cette capacité qu'une fois pour chaque attribut par tour."]),
        ...
        ("Linkuriboh", "<i>Link 1</i>", ["(1 - Activable Flash Soft) Sacrifiez Linkuriboh : une créature attaquante ciblée devient 0/1 jusqu'à la fin du tour.", "(2 - Activable Sorcery Soft) Sacrifiez une créature de valeur de mana 1 : renvoyez Linkuriboh depuis votre Grave sur le champ de bataille."]),
        ...
        ("Salamangreat Almiraj", "<i>Link 1</i>", ["(1 - Activable Flash Soft) Sacrifiez Almiraj : une créature ciblée que vous contrôlez gagne indestructible jusqu'à la fin du tour.", "(2 - Activable Sorcery Soft) Si une créature que vous contrôliez a été détruite ce tour-ci, renvoyez Almiraj depuis votre Grave sur le champ de bataille."]),
        ("Cross-Sheep", "<i>Link 2</i>", ["(1 - Déclenchable Soft) Si une Extra Deck Creature arrive dans une zone pointée par Cross-Sheep, appliquez selon son type : Fusion — renvoyez une créature depuis votre Grave ; Synchro — les créatures que vous contrôlez gagnent +1/+1 ; Xyz — les créatures adverses gagnent -1/-1."]),
        ("Underworld Goddess of the Closed World", "<i>Link 5</i>", ["(1 - Passif) Vous pouvez utiliser une créature adverse comme matériau pour invoquer Underworld Goddess.", "(2 - Déclenchable) Si Underworld Goddess arrive, les créatures adverses perdent toutes leurs capacités jusqu'à la fin du tour.", "(3 - Activable Flash Soft) Contrecarrez une capacité qui cible une carte dans un Grave."]),
